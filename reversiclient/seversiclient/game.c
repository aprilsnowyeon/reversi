...
struct tcpdata_client g_client;
		...
		for (j = 0; j < COLS; j++)
		{
			if (dt.int_grid[i][j] == CLIENT)
			{
				...
				for (k = -1; k <= 1; k++) // 2중 for문 k -1~1, l -1~1
				{
					for (l = -1; l <= 1; l++)
					{
						if (dt.int_grid[i + k][j + l] == SERVER) // 인접한 8방향으로 상대돌이 있는지 확인
						{
							add = 1;
							while (1)
							{
...
main() //뭐넣지 그러게
{
	...
	while (1)
	{
		if (dt.int_myturn == CLIENT) //클라 흑돌 턴
		{
			enablechecking();
			graphic();
			typing();
			...
			if (dt.int_key == 'z')
			{
				if (dt.int_grid[dt.int_x - 1][dt.int_y - 1] == 3) //둘 수 있는곳에 돌 놓기
				{
					...
					client_reverse(); //int_grid 정수값들 대격변
					countingscore(); //점수계산
					graphic(); //화면갱신
		...
		else if (dt.int_myturn == SERVER) //서버 백돌 턴
		{

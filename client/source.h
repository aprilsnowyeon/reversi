...
struct tcpdata_client   // tcp 통신에 사용되는 구조체 
{
    WSADATA wsadata; // 윈소켓
    SOCKET clientSocket2; // 소켓 구조체
    SOCKADDR_IN serverAddress2; // 서버 주소 구조체
    int stringLength2; // 받은 데이터 길이 저장
    char message2[BUFSIZE];  // 보낼 메시지 저장
    char data2[BUFSIZE]; // 받은 메시지 저장
    int clientAddressSize2; // 클라이언트 주소 크기 저장
    int retval;  // 연결요청 상태 저장
};